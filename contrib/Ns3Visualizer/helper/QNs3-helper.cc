...
QNs3Helper::ConfigureVisualizerSampling(bool precise, uint32_t rough)
{
    if (precise)
    {
        setenv("NS3_VISUALIZER_PRECISE", "1", 1);
        setenv("NS3_VISUALIZER_SAMPLE_RATE", "1", 1);
        return;
    }

    unsetenv("NS3_VISUALIZER_PRECISE");
    if (rough > 1)
    {
        setenv("NS3_VISUALIZER_SAMPLE_RATE", std::to_string(rough).c_str(), 1);
        return;
    }
    unsetenv("NS3_VISUALIZER_SAMPLE_RATE");
}

bool
QNs3Helper::LaunchTimelineViewerAsync()
{
    const int ret = std::system("build/Ns3VisualizerApp --timeline-only >/dev/null 2>&1 &");
    return ret == 0;
}
                             ...
                             bool launchViewer)
{
    ...
    const char* disableViewerEnv = std::getenv("NS3_VISUALIZER_DISABLE_VIEWER");
