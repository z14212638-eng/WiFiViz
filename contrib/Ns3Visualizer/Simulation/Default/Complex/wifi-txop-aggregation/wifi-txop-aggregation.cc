...
int main(int argc, char *argv[]) {
  ...
  cmd.AddValue("enable-visualizer", "Enable Ns3Visualizer timeline capture",
               enableVisualizer);
  ...
  NetDeviceContainer allDevices = QNs3Helper::MergeDevices(allApDevices, allStaDevices);
  QNs3Helper::ConfigureVisualizerSampling(precise, rough);
  Ptr<SniffUtils> Sniff_Utils = QNs3Helper::MaybeEnableVisualizer(
      enableVisualizer, allDevices, simulationTime.ToDouble(Time::S), true);
