...
public:
    ...
    std::string GenerateScript()
    {
        ...
        script << "    bool enableVisualizer = false;\n";
        script << "    bool precise = true;\n";
        script << "    uint32_t rough = 1;\n";
        script << "    CommandLine cmd(__FILE__);\n";
        script << "    cmd.AddValue(\"enable-visualizer\", \"Enable Ns3Visualizer timeline capture\", enableVisualizer);\n";
...
NS_LOG_COMPONENT_DEFINE("GeneratedSimulation");
    ...
    Ptr<SniffUtils> Sniff_Utils = QNs3Helper::MaybeEnableVisualizer(enableVisualizer, allDevices, duration, true);
)";
    }
