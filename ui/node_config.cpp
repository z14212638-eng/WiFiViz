...
void node_config::setAvailableTargets(const QVector<QPair<QString, QString>> &targets)
{
...
void node_config::on_pushButton_2_clicked()
{
    ...
    const QString currentStaValue = QString("STA_%1").arg(StaIndex);
    for (const auto &target : m_availableTargets)
    {
        if (target.second != currentStaValue)
        {
